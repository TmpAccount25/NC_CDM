...
def prep_input(x):
    ...
    # # Mask = torch.from_numpy(cs.cartesian_mask(shape, acc=2, sample_n=10, centred=False)) # (1,h,w)
    # # for i in range(shape[0]):
    # #     mask[i] = torch.rot90(Mask[0],k=1,dims=[0,1])
    # # Mask = torch.rot90(torch.fft.ifftshift(Mask[0]),k=1,dims=[0,1])
    # # torch.save(Mask,'mask_5.pth')
    # Mask = torch.load('mask_2.pth')
    # for i in range(shape[0]):
    #     mask[i] = torch.fft.ifftshift(Mask)
        
    # # plt.subplot(121)
    # # plt.imshow(torch.abs(Mask).cpu().detach().numpy(),'gray')
    # # plt.show()
    # under_kspace = mask * torch.fft.fftshift(x)
    # under_kspace = torch.fft.ifftshift(under_kspace)
    # # under_kspace = mask * x
    ...
    # plt.subplot(122)
    # plt.imshow(torch.abs(all_k[0,:,:]).cpu().detach().numpy(),'gray')
    # plt.show()
...
def train_epoch(args, epoch, model, epoch_iterator, optimizer, writer, criterion):
    ...
    for iter, data in enumerate(epoch_iterator):
        ...
        x = st1 - st
        output = myFFT2.real2complex(output)
        # loss = torch.mean(torch.square(torch.abs(output) - gt_img)) 
        loss = criterion(torch.abs(output),gt_img)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
def evaluate(args, epoch, model, epoch_iterator, writer, criterion):
    ...
    #     plt.subplot(133)
    #     plt.imshow(torch.abs(output[0,:,:]).cpu().detach().numpy(),'gray')
    #     plt.show()
...
def create_arg_parser():
    ...
    parser.add_argument('--checkpoint', type=pathlib.Path, required=False,
                        default=r"checkpoints\model.pt", help='resume=True时, 输入模型路径')
    parser.add_argument('--best_model', type=pathlib.Path, required=False,
                        default=r"checkpoints\best_model.pt", help='resume=True时, 输入best_model的路径')
    return parser
