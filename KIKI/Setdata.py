...
class SliceData(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # data = np.fft.fftshift(np.transpose(scio.loadmat(fname)['img_xe'], (2, 0, 1)))
        kspace = data[slice]
...
class Slice7tData(Dataset):
    def __init__(self, root, transform, batch_size):
        ...
        for file in files:
            fname = os.path.join(root, file)
            # data = np.load(fname)
            matdata = h5py.File(fname) ['img_c'][:,:,45:145]
            # print(data.shape, fname)
            if len(matdata.shape) == 3:
                data = np.transpose(matdata, (2, 0, 1))
                # data = np.fft.fftshift(np.transpose(matdata, (2, 0, 1)))
    ...
    def __getitem__(self, i):
        fname, slice = self.example[i]
        data = np.transpose(h5py.File(fname)['img_c'][:,:,45:145], (2, 0, 1))
        # data = np.fft.fftshift(np.transpose(scio.loadmat(fname)['img_xe'], (2, 0, 1)))
        kspace = data[slice]
        kspace = kspace['real'] + 1j*kspace['imag']
        return self.transform(kspace,slice)

class ThreeD_nslice_Data(Dataset):
    def __init__(self, root, transform, batch_size):
        self.num_input_slices = 20
        self.transform = transform
        self.example = []
        files = os.listdir(root)
        for file in files:
            ...
            # data = np.load(fname)
            matdata = scio.loadmat(fname)['img_xe']
            nslice = matdata.shape[2]//self.num_input_slices
            # print(data.shape, fname)
            if len(matdata.shape) == 3:
                data = np.transpose(matdata, (2, 0, 1))
                self.example += [(fname, block) for block in range(nslice)]

    def __len__(self):
        return len(self.example)

    def __getitem__(self, i):
        fname, block = self.example[i]
        data = np.transpose(scio.loadmat(fname)['img_xe'], (2, 0, 1))
        add = int(self.num_input_slices)
        imgs = data[2:2+add, :, :]
        image = imgs
        return self.transform(image)
