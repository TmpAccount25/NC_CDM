...
class Nii_Data(Dataset):
    ...
    def __getitem__(self, i):
        ...
        lung_mask = itk.GetArrayFromImage(imgdata)[0]
        img = data[slice]
        # plt.subplot(121)
...
def Show(model):
        ...
        imgdata = itk.ReadImage(r'H:\KIKI\test\test_gt.nii')
        data = itk.GetArrayFromImage(imgdata)
        ...
        norm_img_recon = metric.normalized(np.abs(out))*lung_mask
        norm_gt_img = metric.normalized(np.abs(gt_img))*lung_mask
        norm_us_img = metric.normalized(np.abs(us_img))*lung_mask

        psnr_recon = metric.psnr_mask(norm_img_recon[0], norm_gt_img[0],lung_mask)
        psnr_zf = metric.psnr_mask(norm_us_img[0], norm_gt_img[0],lung_mask)
...
def test_recon(args, model, data_loader):
    ...
    for iter, data in enumerate(data_loader):
        ...
        # plt.title("us_img")
        # plt.axis('off')
        # plt.imshow(np.abs(us_img[3,:,:]),'gray')    # 全采样图像
        # plt.subplot(142)
        # plt.title("gt_img")
        # plt.axis('off') 
        # plt.imshow(np.abs(gt_img[3,:,:]),'gray')
        # plt.subplot(143)
        # plt.title("recon_img")
        # plt.axis('off')
        # plt.imshow(np.abs(out[3,:,:]),'gray')
        # plt.subplot(111)
        # plt.title("mask")
        # plt.axis('off')
        # plt.imshow(np.abs(mask[3,0,:,:]),'gray')
        # plt.show()
        ...
        shape = out.shape
        for i in range(shape[0]):
            # psnr_recon = metric.psnr_mask(norm_img_recon[i], norm_gt_img[i],lung_mask[i])
            # psnr_zf = metric.psnr_mask(norm_us_img[i], norm_gt_img[i],lung_mask[i])
...
def main(args):
    ...
    for i in range(30):
        ...
        print('i=', i)
    print('recon_psnr=', np.mean(mean_PSNR_recon), 'recon_ssim', np.mean(mean_SSIM_recon), 'zf_psnr', np.mean(mean_PSNR_zf), 'zf_ssim', np.mean(mean_SSIM_zf))
...
def create_arg_parser():
    ...
    parser.add_argument('--checkpoint', type=pathlib.Path, required=False,
                        default=r"Xe\model.pt", help='required=True时, 输入模型路径')
    return parser
